...
(defn draw-player-character [g {:keys [x y hitbox-width hitbox-height]}]
  (let [player-image (load-image "resources/images/basic_stickman_w.png")
        g2d (doto ^Graphics2D g
              (.setRenderingHint RenderingHints/KEY_ANTIALIASING RenderingHints/VALUE_ANTIALIAS_ON))]
    (.drawImage g player-image x y hitbox-width hitbox-height nil)
    ;; Draw debug border
    (.setColor g2d Color/RED)
    (.drawRect g2d x y hitbox-width hitbox-height)))